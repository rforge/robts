...
acfyohai <- function(timeseries,p,maxlag,psifunc=super) {
...
estimate <- try(yohai(timeseries,p,psifunc),silent=TRUE)
...
if (p>1){
acfp[2:(p+1)] <- transformer(werte)} else{acfp[2] <- werte}	# autocorrelations until lag p
...
if (p>1){
acfpaic[2:(p+1)] <- transformer(werte)} else{acfpaic[2] <- werte}	# autocorrelations until lag p
