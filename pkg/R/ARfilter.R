...
ARfilter <- function(timeseries,p,psifunc=smoothpsi) {
n <- length(timeseries)
aicv <- numeric(p)
...
if (p > 1) for (j in 2:p) {
	segam <- function(x) filter(x,helppar,varold,timeseries,psifunc=psifunc)[[1]]
	op <- try(optimize(segam,c(-1,1)),silent=TRUE)
	if (inherits(op,"try-error")){
		warning("Optimization failed. Partial Autocorrelation can not be computed.")
		return(partial)
...
timeseriesalt[,j] <- try(filter(0,partial[1:j],varold,timeseries,psifunc=psifunc)[[2]],silent=TRUE)
...
erg <- list(partial,varold, filter(0,partial,varold,timeseries,psifunc=psifunc)[[2]],phiacf,aicv,timeseriesalt)
